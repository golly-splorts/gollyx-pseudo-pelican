...
case ${GOLLYX_STAGE} in
    ...
    integration)
    DOM="${GOLLYX_BASE_UI}"
    REPO="pseudo.golly.life-integration"
    ;;
    prod)
    DOM="${GOLLYX_BASE_UI}"
    REPO="pseudo.golly.life"
    ;;
    *)
...
(
cd ${GOLLYX_PSEUDO_PELICAN_HOME}/pelican
rm -fr output
